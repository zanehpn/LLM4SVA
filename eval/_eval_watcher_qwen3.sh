#!/bin/bash
# Watch a Qwen3-8B-based SFT/OPD output dir for new checkpoints; when one
# appears, fire a Func@1 eval on Human + Machine and append the result
# to a summary file. Skip checkpoints already evaluated.
#
# Usage:
#   bash _eval_watcher_qwen3.sh <output_dir> <gpu_id> [<base_model>]
set -u
OUT_DIR="${1:?output_dir required}"
GPU="${2:-0}"
BASE_MODEL="${3:-/ssd2/xiacong/models/Qwen_Qwen3-8B}"
